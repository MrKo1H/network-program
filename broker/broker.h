...
void recvQuit(char *pkt, char *usr){
	int idx;

	idx =FIXED_HEADER_SIZE + MESSAGE_ID_SIZE;
	strcpy(usr, pkt +idx);
}
int makeQuit(char *pkt, int msgID){
	int idx;
	idx = FIXED_HEADER_SIZE;
	setMsgID(pkt, idx, msgID);
	setFixedHeader(pkt, 10, idx - FIXED_HEADER_SIZE);
	return idx;
}

void quitUser(struct sub * a[], char *usr_id, int *cnt){
	for( int i = 0; i < *cnt; i++)
		if( !strcmp((*a[i]).s_cli_id, usr_id)){
			a[i] = a[--(*cnt)];
			return;
	}
}
